...
import { TreasureHunterConditionType, type TreasureHunterData } from '@/types/treasureHunter'

const voted = ref<TreasureHunterConditionType>(TreasureHunterConditionType.Null)

const config = useConfigStore()

const state = useStateStore()
// @ts-ignore
const {gameData}: {gameData: Ref<TreasureHunterData>} = storeToRefs(state)

const extension = useExtensionStore()
const axios = extension.axios

function vote(condition: TreasureHunterConditionType) {
  voted.value = condition;
  axios.post(config.getApiUrl(`game/15/${condition}`)).catch(error => {
