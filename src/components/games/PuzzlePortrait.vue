...
// @ts-ignore
const { gameData }: { gameData: Ref<PuzzlePortraitData> } = storeToRefs(state)
const players = gameData.value.players

const extension = useExtensionStore()
const axios = extension.axios

const apiUrl = config.getApiUrl('game/45/')
  ...
  const config: GameConfig = {
    ...
    scene: {
      ...
      create: function () {
        ...
        this.input.on('dragend', (pointer: Pointer, gameObject: Card, dropped: Card) =>
        {
          if (!dropped)
          {
            // @ts-ignore
