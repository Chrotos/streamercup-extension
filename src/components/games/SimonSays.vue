...
import { SimonSaysConditionType, type SimonSaysData } from '@/types/simonSays'

const voted = ref<SimonSaysConditionType>(SimonSaysConditionType.Null)

const config = useConfigStore()

const state = useStateStore()
// @ts-ignore
const {gameData}: {gameData: Ref<SimonSaysData>} = storeToRefs(state)

const extension = useExtensionStore()
const axios = extension.axios


function vote(condition: SimonSaysConditionType) {
  voted.value = condition;
  axios.post(config.getApiUrl(`game/38/${condition}`)).catch(error => {
    if (error.response?.status === 409) {
      voted.value = condition;
      return;
    }

    voted.value = SimonSaysConditionType.Null;
  });
}
</script>
