<template>
  <div style="display: inline-block; width: 100%">
    <h1 style="width: 100%; text-align: center">Hallway Haste</h1>
    <div style="width: 100%">
...
// @ts-ignore
const {gameData}: {gameData: Ref<HallwayHasteData>} = storeToRefs(state)

const voted = ref<number>(-1)

function vote (player: number) {
  voted.value = player;
  axios.post(config.getApiUrl(`game/34/${player}`)).catch(error => {
